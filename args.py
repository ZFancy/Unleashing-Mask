...
def parse_arguments():
    ...
    parser.add_argument("--freeze-conv", action="store_true", help="Whether or not to train only conv (this freezes conv)")
    parser.add_argument("--freeze-fc", action="store_true", help="Whether or not to train only fc (this freezes fc)")
    ...
    parser.add_argument('--prune-type', default="layer_wise", choices=["layer_wise", "model_wise"], help="Prune every layer to a same percentile or prune the parameters as a whole")
    ...
    parser.add_argument('--ood_factor', type=float, default= 1.0, help='ood_dataset_size = len(train_loader.dataset) * ood_factor default = 1.0')
    ...
    parser.add_argument('--final', action='store_true', help='if need to evaluate ood performance')
    ...
    parser.add_argument('--atypical', action='store_true', help='whether to train on typical/atypical samples')
    parser.add_argument('--generate', action='store_true', help='whether to generate subset')
    parser.add_argument('--generate-size', type=int, default=1000, help='The size of the subset')

    # post-hoc ood_method: react, ash, dice
    parser.add_argument('--ood-method', type=str, default='none', choices=['none', 'react', 'ash_p', 'ash_b', 'ash_s'])
    parser.add_argument('--react-threshold', type=float, default=1.0)
    parser.add_argument('--ash-percentile', type=float, default=65)

    # oe ood_method: oe, energy(w.aux), poem, doe
    parser.add_argument('--oe-ood-method', type=str, default='oe', choices=['oe', 'energy', 'poem', 'doe'])
    parser.add_argument('--warmup', type=int, default=5, help='warm up epochs for DOE (default: 5)')
